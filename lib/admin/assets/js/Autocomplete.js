(function($){
	...
	function AutoComplete(
		inputSelectorString, inputGeodataString, callback
	) {
		this.jqInput = $(inputSelectorString);
		this.jqInputGeo = $(inputGeodataString);
				...
				source: function(query, process){
					if(query && query.length > 0){
						$this.askGoogleAPI(query)
							.done($this.askGoogleAPISuccess.bind($this, query, process))
							.fail($this.askGoogleAPIError.bind($this, query, process));
					}else{
						$this.autocompleteSelected = null;
						$this.callback();
					}
	...
	AutoComplete.prototype.setSelected = function(item, inputText, ignoreCallback) {
		...
		this.jqInputGeo.val(JSON.stringify(item));
